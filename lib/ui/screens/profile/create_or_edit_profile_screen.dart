...
const _universities = <String>['MedUni Wien', 'SFU Wien', 'Karl Landsteiner'];
const Map<String, String> _universityCodeMap = <String, String>{
  'MedUni Wien': 'MUW',
  'SFU Wien': 'SFU',
  'Karl Landsteiner': 'KL',
};

const _specializations = <String>['human', 'zahn'];
...
String _mapUniNameToCode(String name) {
  return _universityCodeMap[name] ?? name;
}

String _resolveUniversityName({String? code, String? name}) {
  if (name != null && _universities.contains(name)) {
    return name;
  }
  if (code != null && code.isNotEmpty) {
    for (final entry in _universityCodeMap.entries) {
      if (entry.value.toLowerCase() == code.toLowerCase()) {
        return entry.key;
      }
    }
  }
...
String _normalizeSpecializationValue(String? value) {
  final normalized = (value ?? '').toLowerCase();
  if (_specializations.contains(normalized)) {
    return normalized;
  }
  return _specializations.first;
}
...
class _SelectProfilePictureScreen extends State<CreateOrEditProfileScreen> {
  ...
  Future<void> _navigateAfterProfileUpdate() async {
    ...
    final onboardingController = FirstLoginOnboardingController();
    final shouldShow = await onboardingController.shouldShowOnboarding(uid);
    if (!mounted) return;
    if (shouldShow) {
      await context.pushReplacementNamed(Routes.firstLoginOnboarding);
    } else {
      await context.pushNamedAndRemoveUntil(
        Routes.home,
        predicate: (_) => false,
  ...
  String get _resolvedSpecialization =>
      ...
      _normalizeSpecializationValue(
        _currentUserProfile?.specialization ??
            _currentUserProfile?.studyProgram,
      );

  String get _resolvedSemester =>
      _selectedSemester ??
  ...
  Widget _buildMetaSelectors(UserProfile profile) {
    final colorScheme = Theme.of(context).colorScheme;
    _ensureInitialSelections(profile);
    ...
    return Column(
      ...
      children: [
        ...
        AnimatedCustomDropdown<String>(
          items: _universities,
          initialItem: _resolvedUniversityName,
          decoration: _dropdownDecoration(colorScheme),
          onChanged: (value) {
            if (value == null) return;
            setState(() => _selectedUniversityName = value);
          },
        ),
        ...
        AnimatedCustomDropdown<String>(
          ...
          initialItem: _resolvedSpecialization,
          decoration: _dropdownDecoration(colorScheme),
          onChanged: (value) {
            if (value == null) return;
            setState(() => _selectedSpecialization = value.toLowerCase());
          },
        ),
        const SizedBox(height: 12),
        Text(
        ...
        return BlocConsumer<ReferAndEarnCubit, ReferAndEarnState>(
          listener: (_, referState) async {
            ...
            if (referState is ReferAndEarnSuccess) {
              final selectedUniName = _resolvedUniversityName;
              final selectedSpec = _resolvedSpecialization;
              final selectedSemester = _resolvedSemester;
              final selectedUniCode = _resolvedUniversityCode;
              ...
              context.read<UserDetailsCubit>().updateUserProfile(
                ...
                studyProgram: selectedSpec,
                specialization: selectedSpec,
                semester: selectedSemester,
                universityName: selectedUniName,
                universityCode: selectedUniCode,
              );
              ...
              builder: (updateContext, updateState) {
                ...
                return TextButton(
                  ...
                  onPressed: () async {
                    ...
                    if (selectedSpec.isEmpty || selectedSemester.isEmpty) {
                      UiUtils.showSnackBar(
                        'Bitte Spezialisierung und Semester wählen.',
                        context,
                      );
                      return;
                    }
          ...
          children: [
            BlocConsumer<UserDetailsCubit, UserDetailsState>(
              ...
              builder: (context, state) {
                ...
                _ensureInitialSelections(userProfile);
                    ...
                    children: [
                      ...
                      if (!widget.args.isNewUser) ...[
                        ...
                        Padding(
                          ...
                          child: _buildMetaSelectors(userProfile),
                        ),
                        SizedBox(height: size.height * .03),
                      ] else ...[
